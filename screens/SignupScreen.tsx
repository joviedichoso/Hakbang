...
interface FitnessGoal {
  id: string
  label: string
  icon: keyof typeof FontAwesome.glyphMap // Ensure icon type is correct
  color: string
}

const SignupScreen: React.FC<SignupScreenProps> = ({ navigation, navigateToLanding, navigateToSignIn }) => {
  // Form state
  ...
  const validateStep1 = (): boolean => {
    const newErrors: Partial<FormErrors> = { username: "", email: "", password: "" }
    let hasError = false
  ...
  const validateStep2 = (): boolean => {
    const newErrors: Partial<FormErrors> = { height: "", weight: "" }
    let hasError = false
  ...
  const handleSignUp = async () => {
    ...
    setTimeout(() => {
      ...
      setModalMessage("Your HakbangQuest account has been created successfully. Welcome to your fitness journey!")
  ...
  const handleModalClose = () => {
    ...
    if (modalType === "success") {
      // Use the passed navigateToSignIn prop and pass email as param
      navigateToSignIn()
    }
  }
  ...
  const getBmiCategory = (bmiValue: string) => {
    ...
    if (bmi < 18.5) return { category: "Underweight", color: "#FFD166" }
    if (bmi < 25) return { category: "Normal", color: "#06D6A0" }
              ...
              style={twrnc`w-10 h-10 rounded-full items-center justify-center ${
                currentStep === step
                  ...
                  : currentStep > step
                    ...
                    : "bg-[#1E2538] border-2 border-gray-600"
              }`}
            >
              {currentStep > step ? (
        ...
        <View style={twrnc`mb-6`}>
          {fitnessGoals.map((goal) => (
            ...
            >
              ...
              >
                <FontAwesome name={goal.icon} size={20} color={fitnessGoal === goal.id ? "#FFFFFF" : "#FFFFFF"} />
              </View>
            ...
            {currentStep === 1 && (
              <View style={twrnc`bg-[#1E2538] p-4 rounded-xl border border-gray-700 mt-4`}>
                <View style={twrnc`flex-row justify-center items-center`}>
                  ...
                  <TouchableOpacity
                    onPress={() => navigateToSignIn()} // Use the passed navigateToSignIn prop
                    style={twrnc`ml-1`}
                    activeOpacity={0.7}
                  >
